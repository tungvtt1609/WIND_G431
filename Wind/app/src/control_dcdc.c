...
uint32_t mppt_cnt = 0;
uint32_t Time_Cap = 0, Time =0;
bool Trigger, Pre_Trigger;
float Vout_Sum = 0, Iload_Sum = 0, Vin_Sum = 0, Mean_Vout = 0 , Mean_Iload = 0, Mean_Vin = 0;
static void DcDcProcess(void)
{
	mppt_cnt++;
	Signal_Update(VIN);
...
//	MPPTUpdate(&g_wind.mppt, g_wind.iwt, g_wind.vwt);
	...
	Trigger = Pin_Func_Read(FREQ);
	if ((Trigger == 0 && Pre_Trigger == 1)){
		Time_Cap = Time;
		Time = 0;
	}
	else{
...
static void DcDc_Buck_Boost_Mppt(void){
	...
	if (g_wind.is_plugin == true){
		...
		MPPT_controller_pi_var.Fdbk 	= 	g_wind.mppt.VmppOut;
