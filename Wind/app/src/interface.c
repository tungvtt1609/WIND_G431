...
void Send_Data_To_Broker(void)
{
	if (sending_period == 0)
	{
		...
		if (MQTT_Transmit_Data((void *)&battery_send_data, BATTERY_DATA))
		{
		...
		if (MQTT_Transmit_Data((void *)&wind_send_data, WIND_DATA))
		{
			Pin_Func_TurnOff(LED1);
		}
		else
		...
		if (MQTT_Transmit_Data((void *)&system_send_data, SYSTEM_DATA))
		{
			Pin_Func_TurnOff(LED1);
		}
		else
