...
{
//		GPIO_InitTypeDef SCL_Output={0};
		//RCC_AHBPeriphClockCmd(RCC_AHBPeriph_GPIOA,ENABLE); // User can fix this
		
//		SCL_Output.GPIO_Pin=SCL;
//		SCL_Output.GPIO_Mode=GPIO_Mode_OUT;
//		SCL_Output.GPIO_Speed=GPIO_Speed_Level_3;
//		SCL_Output.GPIO_OType=GPIO_OType_OD;
//		SCL_Output.GPIO_PuPd=GPIO_PuPd_UP;
//
//		GPIO_Init(SCL_PORT,&SCL_Output);
...
{
//		GPIO_InitTypeDef SDA_Output={0};
//		RCC_AHBPeriphClockCmd(RCC_AHBPeriph_GPIOA,ENABLE); // User can fix this
//
//		SDA_Output.GPIO_Pin=SDA;
//		SDA_Output.GPIO_Mode=GPIO_Mode_OUT;
//		SDA_Output.GPIO_Speed=GPIO_Speed_Level_3;
//		SDA_Output.GPIO_OType=GPIO_OType_OD;
//		SDA_Output.GPIO_PuPd=GPIO_PuPd_UP;
		GPIO_SetPinOut(SDA_PORT, SDA);
		Write_SDA(1);
//		GPIO_Init(SDA_PORT,&SDA_Output);
}


void SDA_InputMode(void)
{
//		GPIO_InitTypeDef SDA_Input={0};
//		RCC_AHBPeriphClockCmd(RCC_AHBPeriph_GPIOA,ENABLE); // User can fix this
//
//		SDA_Input.GPIO_Pin=SDA;
//		SDA_Input.GPIO_Mode=GPIO_Mode_IN;
//		SDA_Input.GPIO_Speed=GPIO_Speed_Level_3;
//		SDA_Input.GPIO_OType=GPIO_OType_OD;
//		SDA_Input.GPIO_PuPd=GPIO_PuPd_UP;
//
//		GPIO_Init(SDA_PORT,&SDA_Input);
...
void I2C_Write(uint8_t dat)
{
	...
	// ckeck ACK
		...
		if(flag_eeprom== NACK)  ERROR_I2C(); // Received a bit NACK,go to infinite loop
