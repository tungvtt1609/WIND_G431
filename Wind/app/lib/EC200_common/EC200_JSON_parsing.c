...
bool parse_JSON(char *src_str, char *key, char *value)
{
    ...
    int r;
    jsmn_parser p;
    jsmntok_t t[MAX_TOKEN]; /* We expect no more than 20 tokens */

    jsmn_init (&p);
    r = jsmn_parse (&p, src_str, strlen (src_str), t, sizeof(t) / sizeof(t[0]));
    if (r < 0)
    {
        /*TODO:"Failed to parse JSON: %d\n"*/
        return false;
    }
    /* Assume the top-level element is an object */
    if (r < 1 || t[0].type != JSMN_OBJECT)
    {
        /*TODO:"Object expected\n"*/
        return false;
    }
    ...
    for (i = 1; i < r; i++)
    {
        if (jsoneq (src_str, &t[i], key) == 0)
        {
            int key_len = (t[i + 1].end - t[i + 1].start);
            char *key_pointer = (char*) (src_str + t[i + 1].start);
            for (int a = 0; a < key_len; a++)
            {
                value[a] = *(key_pointer + a);
            }
            i++;
